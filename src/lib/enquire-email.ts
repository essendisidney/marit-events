...
export function enquireAutoReplyHtml(payload: EnquirePayload) {
  ...
  const body = `
    ...
    <p style="margin:0 0 16px;font-size:15px;line-height:1.65;color:#4a433c;font-family:Arial,Helvetica,sans-serif;">
      Thank you for writing to Marit Events. We've received your ${escapeHtml(payload.type || "celebration")} enquiry${payload.location ? ` for ${escapeHtml(payload.location)}` : ""} and will reply within 2 hours with a clear next step.
    </p>
...
export function enquireAutoReplyText(payload: EnquirePayload) {
  ...
  return [
    `Dear ${first},`,
    "",
    `Thank you for writing to Marit Events. We've received your ${payload.type || "celebration"} enquiry${payload.location ? ` for ${payload.location}` : ""} and will reply within 2 hours with a clear next step.`,
    "",
